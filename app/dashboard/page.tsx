                  ...
                  {transactions.slice(0, 5).map((transaction, index) => (
                    <tr key={transaction.id || index} className="border-b last:border-0 hover:bg-muted/50">
                      ...
                      <td className={`py-3 text-sm font-medium text-right ${transaction.type === 'Income' ? 'text-[hsl(var(--income))]' : 'text-[hsl(var(--expense))]'}`}>
                        {transaction.type === 'Income' ? '+' : '-'}{formatCurrency(Math.abs(Number(transaction.total || 0)))}
                      </td>
                    </tr>
                  ))}
                </tbody>
